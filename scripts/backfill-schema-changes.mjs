...
import { moodboards, activities, reactions } from "../src/lib/schema";
...
async function backfill() {
  ...
  try {
    ...
    const boardsWithoutCategory = await db
      ...
      .from(moodboards)
      .where(eq(moodboards.primaryCategory, "Uncategorized"));

    console.log(
      `   Found ${boardsWithoutCategory.length} boards without category`,
    );
    ...
    for (const board of allBoards) {
      const remixesResult = await db
        .select({ count: count() })
        .from(activities)
        .where(eq(activities.boardId, board.id))
