...
function log(level, message, data = '') {
  ...
  console.log(`${color}[${timestamp}]${RESET} ${color}${level.toUpperCase()}${RESET} ${message}`, data);
}
...
async function testApiRoute(method, path, body = null) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      log('error', `${method} ${path} returned ${response.status}`, JSON.stringify(data, null, 2));
...
async function runTests() {
  ...
  log('info', 'Auth Token:', TOKEN ? '✓ Present' : '⚠ Not provided (tests will fail if endpoints require auth)');
  console.log();
  ...
  if (!TOKEN) {
    log('warn', 'Skipping authenticated tests. To test /api/user and /api/boards/create:');
  ...
  if (userResult.ok) {
    ...
    log('error', 'POST /api/user', `Failed (${userResult.status || userResult.error})`);
  }

  if (boardResult.ok) {
    log('success', 'POST /api/boards/create', 'Working ✓');
  } else {
    log('error', 'POST /api/boards/create', `Failed (${boardResult.status || boardResult.error})`);
  }

  console.log();
  log('info', 'For detailed server logs, check:');
  log('info', 'Local: npm run dev and look in terminal output');
  log('info', 'Vercel: https://vercel.com/builders-apps/moodboard-generator/logs\n');
}
