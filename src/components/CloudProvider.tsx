...
async function authFetch(
  ...
  if (_isMiniApp) {
    try {
      ...
      throw new Error(
        "Authentication failed — please reopen the app from Warpcast",
      );
    }
  }
...
export function CloudProvider({ children }: { children: React.ReactNode }) {
  ...
  const sync = useCallback(
    async (localOverride?: Artwork[]): Promise<Artwork[]> => {
      if (!user) return [];
      ...
      try {
        ...
        if (typeof navigator !== "undefined" && !navigator.onLine) {
          const localArtworks =
            localOverride ?? (await loadArtworks().catch(() => []));
          enqueueSync(localArtworks.map((a) => a.id));
          setSyncStatus("offline");
        } else {
          setSyncStatus("error");
  ...
  return (
    <CloudContext.Provider
      value={{ user, syncStatus, lastSyncAt, isMiniApp, signIn, signOut, sync }}
    >
      {children}
    </CloudContext.Provider>
  );
