...
import {
  saveArtwork, loadArtworks, deleteArtwork,
  clearDraft, type Artwork, type CanvasImage, type Orientation,
} from '@/lib/storage';
...
export function useCollectionManager(
  ...
  const saveToCollection = useCallback(async () => {
    const { artworkId, title, caption, canvasImages, dimsW, dimsH, orientation, bgColor, imageMargin, categories } = saveCtx;
    ...
    try {
      thumbnail = await renderThumbnailOffscreen(
        canvasImages, dimsW, dimsH, bgColor, imageMargin,
    ...
    await saveArtwork({
      ...
      images: canvasImages, canvasWidth: dimsW, canvasHeight: dimsH,
  ...
  const loadArtworkForEditing = useCallback((aw: Artwork) => {
    ...
    setters.setProcessedData(aw.images.map((i) => ({ dataUrl: i.dataUrl, naturalWidth: i.naturalWidth, naturalHeight: i.naturalHeight })));
  ...
  const duplicateArtwork = useCallback((aw: Artwork) => {
    ...
    setters.setCaption(aw.caption);
    setters.setCanvasImages(aw.images.map((i) => ({ ...i, id: `img-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` })));
    setters.setProcessedData(aw.images.map((i) => ({ dataUrl: i.dataUrl, naturalWidth: i.naturalWidth, naturalHeight: i.naturalHeight })));
