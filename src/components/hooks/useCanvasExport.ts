...
import {
  renderManualMoodboard, renderMoodboardToBlob,
} from '@/lib/canvas';
...
export function useCanvasExport(config: ExportConfig) {
  const { title, caption, view, canvasImages, dimsW, dimsH, bgColor, imageMargin, moodboardUrl } = config;
  ...
  const castToFarcaster = useCallback(async () => {
    ...
    try {
      ...
      if (view === 'manual' && canvasImages.length > 0) {
        blob = await renderMoodboardToBlobOffscreen(
          canvasImages, title.trim(), caption.trim(), dimsW, dimsH, bgColor, imageMargin, 0.85,
          () => renderMoodboardToBlob(canvasImages, title.trim(), caption.trim(), dimsW, dimsH, bgColor, imageMargin),
        );
    ...
    setCastStatus(null);
    clearDraft().catch(() => {});
  }, [title, caption, view, canvasImages, dimsW, dimsH, bgColor, imageMargin, moodboardUrl]);
  ...
  const downloadManual = useCallback(async () => {
    const url = await renderManualMoodboard(canvasImages, title.trim(), caption.trim(), dimsW, dimsH, bgColor, imageMargin);
    saveImage(url);
    clearDraft().catch(() => {});
  }, [canvasImages, title, caption, dimsW, dimsH, bgColor, imageMargin, saveImage]);

  const printManual = useCallback(async () => {
    const url = await renderManualMoodboard(canvasImages, title.trim(), caption.trim(), dimsW, dimsH, bgColor, imageMargin);
    printUrl(url);
  }, [canvasImages, title, caption, dimsW, dimsH, bgColor, imageMargin, printUrl]);
