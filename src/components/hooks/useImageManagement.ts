...
import {
  compressForStorage, createInitialPlacements, loadImage, renderMoodboard,
} from '@/lib/canvas';
import {
  clearDraft, ensureInLibrary,
  type CanvasImage, type Orientation, type LibraryImage,
} from '@/lib/storage';
...
interface ImageManagementSetters {
  ...
  setProcessedData: React.Dispatch<React.SetStateAction<Array<{ dataUrl: string; naturalWidth: number; naturalHeight: number }>>>;
...
export function useImageManagement(
  ...
  const { canvasImages, orientation, view, isProcessing, title, caption, dims } = deps;
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      setters.setMoodboardUrl(renderMoodboard(loaded, title.trim(), caption.trim()));
      setters.setView('auto-result');
    } catch (err) { console.error(err); }
    finally { setters.setIsProcessing(false); }
  ...
  const addFromLibrary = useCallback((libImages: LibraryImage[]) => {
    ...
    if (view === 'manual' && canvasImages.length > 0) {
      ...
      setters.setProcessedData((prev) => [...prev, ...libImages.map((li) => ({ dataUrl: li.dataUrl, naturalWidth: li.naturalWidth, naturalHeight: li.naturalHeight }))]);
    } else {
      setters.setCanvasImages(newImgs);
      setters.setProcessedData(libImages.map((li) => ({ dataUrl: li.dataUrl, naturalWidth: li.naturalWidth, naturalHeight: li.naturalHeight })));
      setters.setArtworkId(null);
      setters.clearHistory();
    }
    setters.setView('manual');
