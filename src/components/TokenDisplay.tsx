    ...
    <div className="fixed top-4 right-4 z-50 max-w-sm">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 shadow-lg">
          <div className="flex items-center justify-between gap-2">
            <div className="text-sm">
              <p className="font-semibold text-green-900">Auth Token Ready ✅</p>
          ...
          <p className="text-xs text-green-600 mt-2">
            Token copied to clipboard. Use: <code className="bg-green-100 px-1 rounded">npm run test:api</code>
          </p>
        </div>
      )}
      ...
      {isLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 shadow-lg">
          <p className="text-sm text-blue-900 font-semibold">🔄 Initializing SDK...</p>
        </div>
      )}
      ...
      {error && !isLoading && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 shadow-lg">
          ...
          <p className="text-xs text-yellow-700 mt-1">
            Open DevTools (Warpcast Simulator) to access token via: <code className="bg-yellow-100 px-1 rounded">await sdk.quickAuth.getToken()</code>
          </p>
        </div>
      )}
    </div>
